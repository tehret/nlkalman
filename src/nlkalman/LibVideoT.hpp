...
//		                (unsigned long int) getpid());
	...
	/**
	 * @brief Compute PSNR and RMSE between i_vid1 and i_vid2 for each frame
	 *
	 * @param i_vid1 : video 1;
	 * @param i_vid2 : video 2;
	 * @param o_psnr  : will contain the PSNR;
	...
	void computePSNR(
	    Video<T> const& i_vid1
	,   Video<T> const& i_vid2
	,   std::vector<float> &o_psnr
	,   std::vector<float> &o_rmse
	){
		if (i_vid1.sz != i_vid2.sz)
			throw std::runtime_error("VideoUtils::computeFramesPSNR: videos have different sizes");

		o_psnr.resize(i_vid1.sz.frames);
		o_rmse.resize(i_vid1.sz.frames);

		for (unsigned f = 0, i = 0; f < i_vid1.sz.frames; ++f)
		{
			double sum = 0.f;
			for (unsigned k = 0; k < i_vid1.sz.whc; k++, i++)
				sum += ((double)i_vid1(i) - (double)i_vid2(i)) *
						 ((double)i_vid1(i) - (double)i_vid2(i));

			o_rmse[f] = sqrtf(sum / (double) i_vid1.sz.whc);
			o_psnr[f] = 20.f * log10f(255.f / o_rmse[f]);
		}
	...
	,   const T p_max = 255.f
		...
		for (unsigned k = 0; k < i_vid1.sz.whcf; k++)
		{
			...
			float value =  ((float)i_vid1(k) - (float)i_vid2(k));
			o_vidDiff(k) = value;
		}
