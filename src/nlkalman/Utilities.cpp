...
bool comparaisonFirst(
	const pair<float, pair<int,int> > &i_pair1
,	const pair<float, pair<int,int> > &i_pair2
){
...
/**
 * @brief Compute the average standard deviation of a set of patches (when already centered).
 *
 * @param i_Set(p_sP, p_nSimP): set of patches;
 * @param p_sP : size of a patch;
 * @param p_nSimP: number of patches in the set;
 ...
 * @return the average standard deviation of the set
 **/
float computeStdDeviationCentered(
	std::vector<float> const& i_Set
,	const unsigned p_sP
,	const unsigned p_nSimP
,	const unsigned p_nChannels
	...
	for (unsigned c = 0; c < p_nChannels; c++) {
		...
		//! Sample standard deviation (Bessel's correction)
		sigma += std / (float) (p_sP * p_nSimP - 1);
	}
...
,   const float p_rmse
    ...
    file << "************" << endl;
    file << "-PSNR" << " = " << p_psnr << endl;
    file << "-RMSE" << " = " << p_rmse << endl;
    cout << endl;
...
int writingMeasures(
    const char* p_pathName
,   const float p_sigma
,   const std::vector<float> p_psnr
,   const std::vector<float> p_rmse
){
    //! Open the file
    ofstream file;
    ...
    file << "-PSNR" << " = ";
    for (int f = 0; f < p_psnr.size(); ++f) file << p_psnr[f] << " ";
    file << endl;
    file << "-RMSE" << " = ";
    for (int f = 0; f < p_rmse.size(); ++f) file << p_rmse[f] << " ";
    file << endl;
